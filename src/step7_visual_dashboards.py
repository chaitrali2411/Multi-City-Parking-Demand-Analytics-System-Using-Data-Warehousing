"""
STEP 7: VISUAL DASHBOARDS (matplotlib + seaborn + plotly)

Builds charts from Step 2 CSV outputs (no DB required):
...
Run from project root after Step 2:

  python src/step7_visual_dashboards.py
"""
...
def _require_csv(path: Path) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(f"Missing {path}. Run step2_cleaning_preprocessing.py first.")
...
*Figures generated by `src/step7_visual_dashboards.py` (matplotlib + seaborn + plotly; PNG saved at 300 DPI for print).*
"""
